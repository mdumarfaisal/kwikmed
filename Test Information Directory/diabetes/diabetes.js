document.addEventListener("DOMContentLoaded", () => {
    ...
    // Import jsPDF if it's being used via CDN
    ...
    // Store the last prediction results
    let lastPrediction = null;
  
    // Handle the diabetes form submission
    document.getElementById("diabetesForm").addEventListener("submit", function(event) {
      ...
      let prediction = predictDiabetes(inputValues, dataset);
      lastPrediction = { 
        result: prediction, 
        values: inputValues, 
        factors: identifyRiskFactors(inputValues)
      };
      
      displayResults(lastPrediction);
    });
  
    // Information modal handling
    ...
    document.getElementById("downloadBtn").addEventListener("click", function() {
      if (lastPrediction) {
        generateReport(lastPrediction);
      }
    });
    
    // Function to validate inputs
    function validateInputs(inputs) {
      if (inputs.Glucose < 0 || inputs.Glucose > 500) {
        alert("Please enter a valid glucose level (0-500 mg/dL)");
        return false;
      }
      if (inputs.BloodPressure < 0 || inputs.BloodPressure > 300) {
    ...
    function predictDiabetes(input, dataset) {
      ...
      dataset.forEach(entry => {
        let difference = 0;
        for (let key in weightedFeatures) {
          if (key in input && key in entry) {
            difference += Math.abs(input[key] - entry[key]) * weightedFeatures[key];
          }
        }
      ...
      return closestMatch ? closestMatch["Outcome"] : 0;
    }
    ...
    function identifyRiskFactors(input) {
      ...
      if (input.Glucose > 125) {
        factors.push({
          ...
          description: "Your glucose level is above 125 mg/dL, which indicates elevated blood sugar levels."
        });
      }
      ...
      if (input.BMI > 30) {
        factors.push({
          factor: "Elevated BMI",
          description: "Your BMI is above 30, which is in the obese range and increases risk of type 2 diabetes."
        });
      }
      ...
      if (input.Age > 45) {
        factors.push({
          factor: "Age Factor",
          description: "Being over 45 years of age increases the risk of developing diabetes."
        });
      }
      ...
      if (input.DiabetesPedigreeFunction > 0.5) {
        factors.push({
          ...
          description: "Your diabetes pedigree function indicates a potential genetic predisposition."
        });
      }
      ...
      if (input.BloodPressure > 130) {
        factors.push({
          ...
          description: "Your blood pressure is elevated, which is associated with increased diabetes risk."
        });
      }
      
      if (input.Insulin > 150 && input.Glucose > 110) {
        factors.push({
          factor: "Insulin Resistance",
          description: "Having elevated insulin with high glucose suggests potential insulin resistance."
        });
      }
    ...
    function displayResults(prediction) {
      ...
      resultElement.innerHTML = prediction.result ?
        "Prediction: <span style='color: #e74c3c;'>High Risk for Diabetes</span> 😢" :
        "Prediction: <span style='color: #27ae60;'>Low Risk for Diabetes</span> 😊";
      
      // Display risk factors
      factorsElement.innerHTML = "";
      if (prediction.factors.length > 0) {
        let factorsText = prediction.result ? 
          "<h4>Key Risk Factors Identified:</h4>" : 
          "<h4>Health Metrics to Monitor:</h4>";
        
        factorsText += "<ul>";
        prediction.factors.forEach(factor => {
          factorsText += `<li><strong>${factor.factor}:</strong> ${factor.description}</li>`;
        });
        factorsText += "</ul>";
        
        if (prediction.result) {
          factorsText += "<p>Please consult with a healthcare professional for a proper diagnosis and medical advice.</p>";
        } else {
          factorsText += "<p>Continue maintaining healthy lifestyle habits to minimize your risk.</p>";
        }
        
        factorsElement.innerHTML = factorsText;
      } else {
        factorsElement.innerHTML = "<p>No specific risk factors identified based on your inputs.</p>";
      }
    }
